...
class Teacher < Person
  ...
  def self.restore_teachers
    if File.exist?('data/teachers.json') && !File.zero?('data/teachers.json')
      file = File.open('data/teachers.json','r')
      file_json = JSON.parse(file.read)
      file_json.each_key { |key| new(file_json[key]['age'], file_json[key]['specialization'], file_json[key]['name'], file_json[key]['id'],parent_permission: true) }
    end
  end
end
